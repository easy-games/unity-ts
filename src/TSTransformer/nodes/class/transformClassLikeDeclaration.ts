...
import {
	generateMetaForAirshipBehaviour,
	generateMetaForAirshipScriptableObject,
	generateMetaForSerializable,
	getClassDecorators,
} from "TSTransformer/nodes/class/AirshipBehaviour/transformAirshipBehaviourClass";
...
export function transformClassLikeDeclaration(state: TransformState, node: ts.ClassLikeDeclaration) {
	...
	*/
	...
	const isScriptableObjectClass = isAirshipScriptableObjectClass(state, node);
	...
	} else if (isScriptableObjectClass) {
		const isExport = (node.modifierFlagsCache & ModifierFlags.Export) !== 0;
		if (isExport) {
			generateMetaForAirshipScriptableObject(state, node)!;
		}
	} else {
